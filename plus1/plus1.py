...
def get_args():
    ...
    required.add_argument("-f",
                          "--file",
                          required=True,
                          help="file or directory to encrypt/decrypt")
...
def get_blacklist():
    ...
    if not os.path.exists(blacklist_location):
        default_blacklist_location = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "plus1_blacklist.txt.dist")
...
def discover_files(filepaths, recursive_opt, blacklist, ignore_ext_opt,
                   h_files_opt, h_dir_opt):
    ...
    while i < len(filepaths):
        ...
        if not os.path.exists(f):
            print(f"Unexpected error: No such file or directory {f}",
                  file=sys.stderr)
            sys.exit(1)
        if os.path.isdir(f):
